...
species = ["loup-garou","ange","orc","harpie","dragonide","gobelin","nain",
...
desc = ["Les loups-garous adoptent en général une apparence humaine, mais peuvent se transformer (notamment lors des nuits de pleine lune) en énormes loups.",
		"Les anges ne diffèrent des humains que par une paire d'ailes de plus ou moins grande envergure dans leur dos, leur permettant de voler.",
		"Les orcs sont de grands êtres humanoïdes à la peau verte ou grise, aux oreilles pointues et aux yeux sombres.",
		"Les harpies sont de véritables hybrides entre des humains et des oiseaux, ayant des ailes à la place des bras et des pattes munies de serres pour jambes.",
		"Les dragonides peuvent alterner entre une forme de dragon et une forme humaine pouvant conserver certain des traits de l'autre forme (queue, écailles, cornes ...).",
		"Les gobelins sont de petits êtres humanoïdes à la peau verte ou grise, aux oreilles pointues et aux yeux sombres.",
		"Les nains ressemblent à des humains, mais ont un corps généralement plus petit et trapu, et une force physique supérieure.",
		"Les faunes ont une apparence majoritairement humaine, mais aussi des cornes et des pattes de bouc ou chèvre en guise de jambes.",
		"Les elfes se distinguent des humains par des oreilles pointues, une taille moyenne plus importante et une grande longévité.",
		...
		"Les fées sont semblables aux humains, à quelques différences près : une plus petite taille, des ailes d'insectes et des oreilles en pointe.",
...
def species():
	cumulated = {i : (x,desc[i]) for i,x in enumerate(species)}

	with open('species.pickle','wb') as file:
		pickle.dump(cumulated,file)
...
def apts():
	class_dict = {l : {'name' : classes_names[i], 'desc' : classes_desc[i]} for i,l in enumerate(classes_letters)}
	power_dict = {l+'n' : {'name' : powers_names[i], 'desc' : powers_desc[i]} for i,l in enumerate(classes_letters)}
	apts = {'classes' : class_dict, 'powers' : power_dict}

	with open('apts.pickle','wb') as file:
		pickle.dump(apts,file)